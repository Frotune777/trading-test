...
class TrendPillar(BasePillar):
    ...
    def analyze(self, snapshot: LiveDecisionSnapshot, context: SessionContext) -> Tuple[float, str, dict, str]:
        ...
        if snapshot.ltp > snapshot.sma_200:
            daily_score += 10
            explanation_parts.append("Price is above the 200-day SMA, which is a long-term bullish signal.")
        else:
            explanation_parts.append("Price is below the 200-day SMA, which is a long-term bearish signal.")

        if snapshot.sma_50 > snapshot.sma_200:
            daily_score += 10
            explanation_parts.append("The 50-day SMA is above the 200-day SMA (a 'golden cross'), which is a medium-term bullish signal.")
        else:
            explanation_parts.append("The 50-day SMA is below the 200-day SMA (a 'death cross'), which is a medium-term bearish signal.")

        if snapshot.ltp > snapshot.sma_50:
            daily_score += 10
            explanation_parts.append("Price is above the 50-day SMA, which is a short-term bullish signal.")
        else:
            explanation_parts.append("Price is below the 50-day SMA, which is a short-term bearish signal.")
        ...
        if snapshot.sma_20_weekly:
            ...
            if snapshot.ltp > snapshot.sma_20_weekly:
                weekly_score = 30
                explanation_parts.append("Price is above the 20-week SMA, confirming the bullish trend on a longer timeframe.")
            else:
                explanation_parts.append("Price is below the 20-week SMA, confirming the bearish trend on a longer timeframe.")
