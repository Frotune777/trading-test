...
def download_historical_data(symbol: str, period: str, interval: str, source: str = 'auto'):
    """Download historical data from selected source."""
    try:
        df = nse.get_historical_prices(symbol, period=period, interval=interval, source=source)
        return df
...
# ... (chart theme remains same)

# Recommendation Logic (Frontend-side for now, or calling backend service)
def get_recommendations(strategy='balanced'):
    try:
        # Ideally fetch from API: requests.get(f"{API_URL}/recommendations?strategy={strategy}")
        # But for standalone demo, we instantiate service direct or use a helper
        # Since we are in same repo, let's use the service class directly for speed
        from backend.app.services.recommendation_service import RecommendationService
        service = RecommendationService()
        return service.generate_recommendations(strategy=strategy, limit=10)
    except Exception as e:
        st.error(f"Error fetching recommendations: {e}")
        return []

# TAB 8: Analysis (Recommendation Engine)
with tab8:
    st.markdown('<p class="sub-header">🧠 AI Recommendation Engine</p>', unsafe_allow_html=True)
    
    col1, col2 = st.columns([1, 3])
    with col1:
        strategy = st.selectbox(
            "Strategy", 
            ["balanced", "growth", "value", "momentum"],
            help="Balanced: 50/50 Tech/Fund\nGrowth: High Revenue\nValue: Low PE\nMomentum: Trend Follow"
        )
        
        if st.button("🚀 Run Analysis", width='stretch'):
            with st.spinner("Analyzing market data..."):
                recs = get_recommendations(strategy)
    ...
    with col2:
        if 'recommendations' in st.session_state:
            ...
            if not recs:
                ...
                for r in recs:
                    with st.expander(f"**{r['symbol']}** - Score: {r['smart_score']}/100 ({r['action']})", expanded=True):
                        # Scores
                        c1, c2, c3 = st.columns(3)
                        c1.metric("Smart Score", r['smart_score'])
                        c2.metric("Technical", f"{r['technical_score']:.1f}")
                        c3.metric("Fundamental", f"{r['fundamental_score']:.1f}")
                        
                        # Progress Bar
                        st.progress(int(r['smart_score']))
                        
                        # Explanation
                        st.markdown(f"**Why?** {r.get('explanation', 'No explanation')}")

                        # Trade Levels
                        if r.get('stop_loss') or r.get('target_price'):
                            st.divider()
                            l1, l2, l3 = st.columns(3)
                            l1.caption("Entry Signal")
                            l1.markdown(f"**{r.get('action')}**")
                            
                            if r.get('stop_loss'):
                                l2.caption("Stop Loss")
                                l2.markdown(f"**{r['stop_loss']}**", help="Suggested stop loss based on ATR")
                                
                            if r.get('target_price'):
                                l3.caption("Target")
                                l3.markdown(f"**{r['target_price']}**", help="Suggested target (1:2 Risk/Reward)")
                        
                        # Details
                        st.caption(f"Strategy: {r['strategy'].title()} | Trend: {r['technical_details'].get('trend', 'N/A')}")
        else:
            st.info("👈 Select a strategy and click 'Run Analysis'")
    ...
    if not selected_symbol:
        ...
        if download_type == "📊 Historical (Since 1990)":
            ...
            with col3:
                ...
                data_source = st.selectbox("Source", 
                    ["🔄 Auto (NSE→Yahoo)", "🇮🇳 NSE Only", "🌍 Yahoo Only"],
                    index=0,
                    help="Auto: Tries NSE first, falls back to Yahoo\nNSE: Official Indian exchange data\nYahoo: Global coverage, reliable"
                )
            with col4:
                st.write("")
                st.write("")
            ...
            if st.button("📥 Download", width='stretch', type="primary"):
                ...
                source_param = source_map[data_source]
                
                with st.spinner(f"Downloading {interval} data from {data_source}..."):
                    df = download_historical_data(selected_symbol, period, interval, source_param)
                    ...
                    if df is not None and not df.empty:
                        ...
                        with col2:
                            ...
                            if 'date' in df.columns:
                                st.metric("From", df['date'].min().strftime('%Y-%m-%d'))
                            elif hasattr(df.index, 'min') and hasattr(df.index.min(), 'strftime'):
                                st.metric("From", df.index.min().strftime('%Y-%m-%d'))
                            else:
                                st.metric("From", "N/A")
                        with col3:
                            if 'date' in df.columns:
                                st.metric("To", df['date'].max().strftime('%Y-%m-%d'))
                            elif hasattr(df.index, 'max') and hasattr(df.index.max(), 'strftime'):
                                st.metric("To", df.index.max().strftime('%Y-%m-%d'))
                            else:
                                st.metric("To", "N/A")
                        with col4:
