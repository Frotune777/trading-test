  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-xs uppercase tracking-[0.2em] font-black text-muted-foreground">Pillar Contribution Breakdown</h3>
        <span className="text-[10px] text-muted-foreground font-mono">WEIGHT-ADJUSTED CALCULATION</span>
      </div>
        ...
        {pillarList.map((pillar) => {
          ...
          return (
            <Card key={pillar.id} className={cn(
              "bg-card border-border relative overflow-hidden group hover:border-sidebar-accent transition-colors",
              ...
              <CardContent className="p-4">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2">
                    <div className="p-1.5 bg-muted rounded border border-border text-muted-foreground group-hover:text-foreground">
                      {PILLAR_ICONS[pillar.id] || <HelpCircle className="w-4 h-4" />}
                    </div>
                    <span className="text-xs font-bold text-foreground uppercase tracking-wider">{pillar.name}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    {pillar.is_placeholder && (
                      <span className="text-[9px] bg-muted text-muted-foreground px-1.5 py-0.5 rounded font-bold uppercase">PLACEHOLDER</span>
                    )}
                    <span className="text-[10px] text-muted-foreground font-mono">{(pillar.weight * 100).toFixed(0)}% WT</span>
                  </div>
                </div>
                ...
                {/* Explanation Text */}
                <div className="text-[11px] leading-relaxed text-muted-foreground line-clamp-2 h-8 italic border-t border-border/50 pt-2 group-hover:text-foreground transition-colors overflow-hidden">
